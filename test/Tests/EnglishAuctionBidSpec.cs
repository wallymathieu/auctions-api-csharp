...
public class EnglishAuctionBidSpec
{
    static Bid ValidBid(Auction auction) =>
        new(User: Buyer, Amount: Amount.Parse("SEK10"), At: auction.StartsAt.AddHours(1.0));

    static Bid BidWithSameUser(Auction auction) => ValidBid(auction) with { User = Seller };

    static Bid BidAfterAuctionEnded(Auction auction) =>
        ValidBid(auction) with
        {
            At = auction.Expiry.AddHours(1.0),
        };
    ...
    public void bid_before_auction_has_started()
    {
        ...
        Assert.Equal(
            ...
            BidBeforeAuctionStarted(auction).Validate(auction)
        );
    }
}
