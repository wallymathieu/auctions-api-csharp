...
public class ApiFixture(IDatabaseFixture databaseFixture, IApiAuth auth)
    : IDisposable,
        IAsyncLifetime
{
    ...
    public async Task<HttpResponseMessage> PostAuction(
        string auctionRequest,
        AuthToken authToken
    ) =>
        await _testServer!
            .CreateRequest("/auctions")
            .And(r =>
            {
    ...
    public async Task<HttpResponseMessage> PostBidToAuction(
        long id,
        string bidRequest,
        AuthToken authToken
    ) =>
        await _testServer!
            .CreateRequest($"/auctions/{id}/bids")
            .And(r =>
            {
    ...
    public async Task InitializeAsync()
    {
        ...
        var application = _webApplicationFactory.WithWebHostBuilder(builder =>
        {
        ...
        using var serviceScope = application
