...
public class JwtAuthAndMsSqlApiFixture() : ApiFixture(new MsSqlDatabaseFixture(), new JwtApiAuth());
...
public class ApiSyncSpecJwtTokenSqlLite(JwtAuthAndSqlLiteApiFixture fixture)
    : ApiSyncSpec(fixture),
        IClassFixture<JwtAuthAndSqlLiteApiFixture>,
...
public abstract class BaseApiSpec(ApiFixture application)
{
    ...
    public async Task Create_auction_1()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.Equal(
                WithId(JToken.Parse(FirstAuctionResponse), id).ToString(Formatting.Indented),
    ...
    public async Task Create_auction_2()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.True(response.IsSuccessStatusCode);
            Assert.Equal(
                WithId(JToken.Parse(SecondAuctionResponse), id).ToString(Formatting.Indented),
                JToken.Parse(stringContent).ToString(Formatting.Indented)
            );
        });
    }
    ...
    public async Task Place_bid_as_buyer_on_auction_1()
    {
        ...
        var bidResponse = await application.PostBidToAuction(
            id,
            """{"amount":"VAC11"}""",
            AuthToken.Buyer1
        );
            ...
            Assert.Equal(
                WithId(
                        HasEnded(
                            WithPriceAndWinner(
                                WithBid(
                                    JToken.Parse(FirstAuctionResponse),
                                    "VAC11",
                                    "#1",
                                    "02:00:00"
                                ),
