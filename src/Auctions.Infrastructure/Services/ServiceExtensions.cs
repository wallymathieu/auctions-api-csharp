...
{
    internal static IServiceCollection AddAuctionServicesImplementation(
    ...
    )
    {
        ...
        services.TryAddScoped<InnerService<ICreateAuctionCommandHandler>>(
            c => new InnerService<ICreateAuctionCommandHandler>(
                c.GetRequiredService<CreateAuctionCommandHandler>()
            )
        );
        services.TryAddScoped<InnerService<ICreateBidCommandHandler>>(
            c => new InnerService<ICreateBidCommandHandler>(
                c.GetRequiredService<CreateBidCommandHandler>()
            )
        );
        return services;
    }
