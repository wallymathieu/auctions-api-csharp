...
///     as "pure" in the sense that it does not depend on or directly invoke any external code as long as you marry entity
///     framework
...
public class AuctionQuery(AuctionDbContext dbContext) : IAuctionQuery
{
    ...
    {
        return await dbContext.GetAuction(auctionId, cancellationToken);
    }

    public virtual async Task<IReadOnlyCollection<Auction>> GetAuctionsAsync(
        CancellationToken cancellationToken = default)
    {
        return await dbContext.GetAuctionsAsync(cancellationToken);
    }
}