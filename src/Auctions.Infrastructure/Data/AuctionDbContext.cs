...
public class AuctionDbContext : DbContext
{
    ...
    private static PropertyBuilder<T> WithUserId<T>(PropertyBuilder<T> self) =>
        self.HasConversion(new ValueConverter<UserId, string>(v => v.Id!, v => new UserId(v)))
            .HasMaxLength(2000);
    ...
    /// </summary>
    public async ValueTask<IReadOnlyCollection<Auction>> GetAuctionsAsync(
    ...
    /// </summary>
    public async ValueTask<Auction?> GetAuction(
        AuctionId auctionId,
        CancellationToken cancellationToken = default
    )
    {
        var auction = await Auctions.FindAsync(
            keyValues: [auctionId],
            cancellationToken: cancellationToken
        );
        if (auction is not null)
            await Entry(auction).Collection(p => p.Bids).LoadAsync(cancellationToken);
        return auction;
    }
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        {
            entity
                .HasDiscriminator(b => b.AuctionType)
                .HasValue(AuctionType.SingleSealedBidAuction);
